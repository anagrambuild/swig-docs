...
`PREPARED`, `SUBMITTED`, `LANDED`, `FAILED`, or `EXPIRED`, and a
`solanaSignature` once one exists. A transfer settles only at `confirmed` or
better, so a `processed` signature still reads as in-flight.
