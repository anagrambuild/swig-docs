...
client authority signatures; responses include signature requests when the
client must sign.
...
is the hex-encoded message to sign.
...
The response is prepared only. Your application supplies any requested client
authority signature, then submits the transactions in the returned order —
directly, or through the [Paymaster API](/developer-api/paymaster).
