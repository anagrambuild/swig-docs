...
The x402 preparation route does not accept a top-level `feePayer`. It uses
`paymentRequired.accepts[].extra.feePayer` from the selected payment
requirement.
...
transaction from a decoded x402 version 2 `PaymentRequired` challenge.
...
When `acceptedIndex` is omitted, Swig selects the first eligible exact-SVM
requirement matching `network`. When provided, Swig validates that entry
without falling back to another requirement.

On success, the response always contains the resolved index, including index
zero:
