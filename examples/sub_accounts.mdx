...
const safeWithdrawIx = await getWithdrawFromSubAccountInstructions(
  ...
  roleId,
  {
    ...
    allowBelowRentExempt: false // Default behavior
  }
);
```
...
const overrideWithdrawIx = await getWithdrawFromSubAccountInstructions(
  swig,
  roleId,
  {
    amount: 1000000n,
...
```typescript
// Token withdrawals work as before
const tokenWithdrawIx = await getWithdrawFromSubAccountInstructions(
  swig,
  roleId,
  {
    amount: 1000000n,
...
- Prevention of unsafe withdrawals
- Explicit override functionality
- Simple withdrawals without additional parameters
