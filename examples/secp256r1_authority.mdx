...
async function setupSecp256r1Transfer() {
  ...
  // 8. Load the created Swig and get wallet address
  const swig = await Swig.fromAccountAddress(svm, swigAccountAddress);
  const swigWalletAddress = await getSwigWalletAddress(swig);
  
  // 9. Create transfer instruction (use swigWalletAddress)
...
<Card title="TypeScript secp256r1 Transfer" icon="code" href="https://github.com/anagrambuild/swig-ts/blob/main/examples/basic/classic/transfer-r1.ts">
