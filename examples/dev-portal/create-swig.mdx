...
<Note>
  This page covers the Developer Portal REST endpoint and its
  <code>@swig-wallet/api</code> client. For wallet creation with the TypeScript
  or Python Developer SDK, use [Create Wallets](/developer-sdk/create-wallets).
</Note>
...
- A [policy template](./create-policy) configured (with or without a signer)
