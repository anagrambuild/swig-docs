---
title: "Legacy Wallet Create API"
sidebarTitle: "Wallet Create API"
---
...
<Note>
  ...
  <code>@swig-wallet/api</code> and <code>@swig-wallet/developer</code>. If you
  want the current prepare-first SDK
  (<code>@swig-wallet/developer-sdk</code> and
  <code>swig-developer-sdk</code>, in
  [anagrambuild/swig-developer-sdk](https://github.com/anagrambuild/swig-developer-sdk)),
  start with the [Server-Side Developer SDK](/developer-sdk).
  Treat <code>@swig-wallet/developer</code> as the legacy developer SDK for
...
- (Optional) A paymaster public key — see [Creating a Paymaster](./create-paymaster)

<Info>
  A paymaster is **optional**. When you omit `paymasterPubkey`, the API returns an unsigned transaction instead of submitting it on-chain. You can then sign and send the transaction yourself. See [Creating Without a Paymaster](#creating-without-a-paymaster) below.
</Info>
...
If you are starting a new server-assisted integration, prefer
<code>@swig-wallet/developer-sdk</code> over
<code>@swig-wallet/developer</code>.

The current package is prepare-first:

1. your server uses an API key
2. the server prepares wallet transactions
3. application-owned signing code satisfies the transaction signer set and any
...
Do not use an empty `created.clientAuthorityTransactions` array to infer that
no application signature is needed. That collection tracks explicit embedded
signature requests; inspect each serialized transaction's required signer set
as well. A plain create normally requires only the fee payer, while policy-led
follow-up instructions can add other required signers.

Use this page when you specifically want the developer-portal create-wallet
endpoint or its older portal clients. Use the [Server-Side Developer SDK](/developer-sdk)
for the current product surface.

For the current hosted operation flows after wallet creation, see
[Transfers and Swaps](/developer-sdk/transfers-and-swaps) and
[Read Wallet State](/developer-sdk/wallet-reads).
...
| Feature | `@swig-wallet/api` | `@swig-wallet/developer` (legacy) |
|---------|---------------------|-----------------------------------|
...
| Best for | Low-level control | Existing legacy portal integrations |
