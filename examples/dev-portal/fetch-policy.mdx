...
<Note>
  ...
  the [Server-Side Developer SDK](/developer-sdk).
...
// Current SDK authority uses the WalletAuthority oneof.
console.log(policy.authority);
console.log(policy.actions);
```

Unlike the portal response above, the current Developer SDK calls the API
Wallet service at `GET /wallet/policies/{policyId}`. Its TypeScript client
preserves the ProtoJSON wallet-authority oneof:
...
The types below describe the portal REST endpoint and its portal-facing
clients. They do not describe the current Developer SDK's ProtoJSON response
shown above.
