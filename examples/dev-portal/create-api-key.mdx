...
API keys authenticate your trusted server when it calls Swig's hosted API.
Create a key in the Developer Portal and keep it in your server's secret
storage. Wallet operations still require the appropriate authority's signature
or approval proofs; the API key does not grant wallet signing authority.

## What API Keys Are Used For

API keys are required for:

- Reading policy template details via the API
...
- [Create a policy template](./create-policy) for wallet-creation configuration
- [Read policy templates](./fetch-policy) programmatically
- [Create wallets](/developer-sdk/create-wallets) with the Developer SDK
