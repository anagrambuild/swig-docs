...
The TypeScript Developer SDK throws `SwigDeveloperSdkError`. Inspect its
HTTP status and code when handling failed API calls. See
[Developer API Errors](/developer-api/errors) for response formats and status
handling.

```typescript
import { SwigClient, SwigDeveloperSdkError } from '@swig-wallet/developer-sdk';
...
The API SDK supports retry configuration:
