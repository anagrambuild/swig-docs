...
<Note>
  These are the raw developer-portal endpoints. Higher-level SDK surfaces live
  separately in <code>@swig-wallet/developer-sdk</code>,
  <code>@swig-wallet/developer</code>, and <code>@swig-wallet/api</code>.
  In the current docs model, <code>@swig-wallet/developer</code> is treated as
  the legacy developer SDK.
</Note>
...
<Info>
  You can specify the wallet authority in two ways: either by providing a `signerId` from the dashboard, or by supplying a `walletAddress` and `walletType` directly. See [Creating a Swig Wallet](./create-swig#specifying-the-wallet-authority) for details.
</Info>
...
Use these portal clients if you want SDK wrappers around the raw endpoints.
For the current prepare-first Swig Developer SDK, see
