...
const walletAddress = await swig.walletAddress();
...
const signInstructions = await getSignInstructions(
  ...
  [
    SystemProgram.transfer({
      fromPubkey: await swig.walletAddress(), // Auto-resolves correct address
      toPubkey: recipient,
...
async function demonstrateV2Features() {
  ...
  console.log(`Wallet address: ${(await swig.walletAddress()).toBase58()}`);
  ...
  const walletBalance = await connection.getBalance(await swig.walletAddress());
