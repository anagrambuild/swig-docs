...
| secp256k1 | Applies `personal_sign` to the lowercase 64-character hex challenge |
...
- A ParticipantSet is not a reusable approval for unrelated transactions.
- Approvals from different plans or shared nonces cannot be mixed.
- The detached signer helpers never receive your Swig API key and do not call
  the hosted API.
- Compilation does not replace transaction-level signatures such as the fee
  payer or another required Ed25519 signer.
