...
for (const balance of tokens.balances) {
  console.log(balance.assetKind, balance.tokenSymbol, balance.uiAmount);
}
...
`limit` is optional. Omit it to take the backend default.
...
These are API-key routes. If you need them in the browser, expose them through
your own proxy — see [Use Proxy Helpers](/developer-sdk/framework-proxies).
