...
See [Permissions and Guardian Recovery](/developer-sdk/permissions) for the
complete Developer SDK action list, required fields, and units. That reference
also explains a recovery role limited to replacing one role's key and a
possible extension requiring multiple guardian approvals, including the
remaining SDK integration work.
