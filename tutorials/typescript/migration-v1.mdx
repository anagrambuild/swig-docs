...
const swig = Swig.fromRawAccountData(swigAddress, accountData);
```
...
// Fetching with null handling (use try/catch)
try {
  const swig = await fetchSwig(connection, swigAddress);
} catch (error) {
  // Handle case where Swig doesn't exist
  const swig = null;
}

// Creating from raw data (equivalent functionality in @swig-wallet/lib)
// Check the lib package documentation for raw data handling
```
