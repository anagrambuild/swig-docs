...
This tutorial covers transaction signing after your wallet and authorities are
already in place.

Finish the earlier quickstart pages first, then create the next tutorial file:

Create `tutorial-3.ts` in `examples/tutorial/classic` for the `classic`
version or in `examples/tutorial/kit` for `kit`:
